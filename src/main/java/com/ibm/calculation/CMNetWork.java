...
public class CMNetWork {
    //数据字典
    private static final Map<String,String> dictionaryCMSsInfoMap = new HashMap<>();
    //初始化数据字典
    static {
        try {
            String record;
            // 设定UTF-8字符集，使用带缓冲区的字符输入流BufferedReader读取文件内容
            CMNetWork cmNetWork = new CMNetWork();
            //BufferedReader file = new BufferedReader(new InputStreamReader(new FileInputStream(filePath), "UTF-8"));
            BufferedReader file = cmNetWork.readCmSsInfoFile();
            // file.readLine(); //跳过表头所在的行
            // 遍历数据行并存储在名为records的ArrayList中，每一行records中存储的对象为一个String数组
            while ((record = file.readLine()) != null) {
                String fields[] = record.split(",");
                dictionaryCMSsInfoMap.put(fields[0],fields[1]+fields[2].substring(6,fields[2].length()));
            }
    ...
    public BufferedReader readCmSsInfoFile(){
        //联通为culist
        //移动为cmlist
        InputStream inputStream = this.getClass().getClassLoader().getResourceAsStream("cm_ss_info.csv");
        // 设定UTF-8字符集，使用带缓冲区的字符输入流BufferedReader读取文件内容
    ...
    public static void main(String[] args) {
        ...
        producerProperties.setProperty("bootstrap.servers", Constants.KafkaAddress.CM_HZ_KAFKA_PROD.getValue());
        producerProperties.setProperty("group.id", "network_send_test");

        if (null == dictionaryCMSsInfoMap){
            try {
        ...
        DataStreamSource<String> dataStreamSource = env.
                addSource(new FlinkKafkaConsumer<>(Constants.NetWorkTopic.cm_hz_ca_report.getTopic(), new SimpleStringSchema(), properties));
        dataStreamSource.map(messageOld -> {
            ...
            while (keys.hasNext()) {
                ...
                //key 是ip+端口
                if (dictionaryCMSsInfoMap.containsKey(splitKey[0])){
                    jsonObjectNew.put("stripe",dictionaryCMSsInfoMap.get(splitKey[0]));
                }
