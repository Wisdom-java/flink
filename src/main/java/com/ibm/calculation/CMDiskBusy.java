...
public class CMDiskBusy {

    //数据字典
    private static final Map<String,String> dictionaryCMDiskMap = new HashMap<>();
    //初始化数据字典
    static {
        try {
            String record;
            // 设定UTF-8字符集，使用带缓冲区的字符输入流BufferedReader读取文件内容
            CMDiskBusy cmDiskBusy = new CMDiskBusy();
            //BufferedReader file = new BufferedReader(new InputStreamReader(new FileInputStream(filePath), "UTF-8"));
            BufferedReader file = cmDiskBusy.readDiskFile();
            // file.readLine(); //跳过表头所在的行
            // 遍历数据行并存储在名为records的ArrayList中，每一行records中存储的对象为一个String数组
            while ((record = file.readLine()) != null) {
                String fields[] = record.split(",");
                dictionaryCMDiskMap.put(fields[3]+fields[4],fields[1]+","+fields[2]);
            }
    ...
    public BufferedReader readDiskFile(){
        //联通为culist
        //移动为cmlist
        InputStream inputStream = this.getClass().getClassLoader().getResourceAsStream("cmlist.csv");
        // 设定UTF-8字符集，使用带缓冲区的字符输入流BufferedReader读取文件内容
    ...
    public static void main(String[] args) {
        ...
        DataStreamSource<String> dataStreamSource = env.
                addSource(new FlinkKafkaConsumer<>(Constants.DiskBusyTopic.cm_tj_cs_metricbeat.getTopic(), new SimpleStringSchema(), properties));
        dataStreamSource.map(message -> {

            JSONObject jsonObject = new JSONObject(message);
            if (null == dictionaryCMDiskMap){
            ...
            //得到对应的值
            String value = dictionaryCMDiskMap.get(key);
            if (null == value){
                return message;
            }
