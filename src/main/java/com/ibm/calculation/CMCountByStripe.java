...
import com.ibm.vo.CountByStripe;
...
public class CMCountByStripe {

    private static final SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");

    //数据字典
    private static final Map<String,String> dictionaryCmSsSetStripeMap = new HashMap<>();
    //初始化数据字典
    static {
        try {
            String record;
            // 设定UTF-8字符集，使用带缓冲区的字符输入流BufferedReader读取文件内容
            CMCountByStripe cmCountByStripe = new CMCountByStripe();
            BufferedReader file = cmCountByStripe.readCuSsInfoFile();
            //跳过表头所在的行
            file.readLine();
            ...
            while ((record = file.readLine()) != null) {
                ...
                dictionaryCmSsSetStripeMap.put(fields[0],"set" + set + stripe);
            }
    ...
    public BufferedReader readCuSsInfoFile(){
        InputStream inputStream = this.getClass().getClassLoader().getResourceAsStream("cm_ss_set_stripe.csv");
        // 设定UTF-8字符集，使用带缓冲区的字符输入流BufferedReader读取文件内容
    ...
    public static void main(String[] args) {
        ...
        properties.setProperty("bootstrap.servers", Constants.KafkaAddress.CM_NJ_KAFKA_PROD.getValue());
        //properties.setProperty("bootstrap.servers", "localhost:9092");
        //properties.setProperty("zookeeper.connect", "localhost:2181");
        properties.setProperty("group.id", "repair_data");
        //cm_tj_cs_metricbeat
        FlinkKafkaConsumer<String> stringFlinkKafkaConsumer = new FlinkKafkaConsumer<>(Constants.countByStripe.cm_nj_cs_report.getTopic(),
                new SimpleStringSchema(), properties);
        Properties producerProperties = new Properties();
        producerProperties.setProperty("bootstrap.servers", Constants.KafkaAddress.CM_NJ_KAFKA_PROD.getValue());
        ...
        SingleOutputStreamOperator<CountByStripe> stringSingleOutputStreamOperator = env.addSource(stringFlinkKafkaConsumer).flatMap(new FlatMapFunction<String, CountByStripe>() {
            @Override
            public void flatMap(String s, Collector<CountByStripe> collector) throws Exception {
                ...
                if (jsonObject.has("message") && jsonObject.has("beat")){
                    ...
                    //set stripe_num
                    countByStripe.setStripeNum(dictionaryCmSsSetStripeMap.get(hostName));
        ...
        stringSingleOutputStreamOperator
                .assignTimestampsAndWatermarks(new BoundedOutOfOrdernessTimestampExtractor<CountByStripe>(Time.seconds(120)) {
                    @Override
                    public long extractTimestamp(CountByStripe countByStripe) {
                        return countByStripe.getTimestamp();
                    }
                })
                .keyBy("stripeNum")
                .timeWindow(Time.seconds(60))
                .apply(new WindowFunction<CountByStripe, String, Tuple, TimeWindow>() {
                    @Override
                    public void apply(Tuple tuple, TimeWindow timeWindow, Iterable<CountByStripe> iterable, Collector<String> collector) throws Exception {
                        //List collectorAll = new ArrayList();
                        List<CountByStripe> list = IteratorUtils.toList(iterable.iterator());
                        int count = list.size();
                        ...
                        for (int i =0 ; i < count; i++){
                            CountByStripe countByStripe = list.get(i);
                        ...
                        jsonObject.put("time_interval",list.get(0).getTimeStr() + " -- " + list.get(list.size() - 1).getTimeStr());
